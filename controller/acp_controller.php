...
class acp_controller
{
	...
	public function display_options()
	{
		...
		$this->language->add_lang('common', 'senky/groupspp');
		...
		if ($this->request->is_set_post('submit'))
		{
			...
			if (empty($errors))
			{
				...
				$this->config->set('senky_groupspp_goodbye', $this->request->variable('senky_groupspp_goodbye', 0));

				// Add option settings change action to the admin log
				$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_ACP_GROUPSPP_SETTINGS');
		...
		$this->template->assign_vars(array(
			...
			'SENKY_GROUPSPP_GOODBYE'	=> (bool) $this->config['senky_groupspp_goodbye'],
		));
	}
